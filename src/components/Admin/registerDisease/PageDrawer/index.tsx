...
const PatologiaDrawer: React.FC<PatologiaDrawerProps> = ({
  isOpen,
  onClose,
  patologia
}) => {
  ...
  useEffect(() => {
    ...
    db.onValue(refSintomas, (snapshot) => {
      const resultadoSintomas = snapshot.val()
        ...
        <DrawerBody>
          <Stack>
            <Box>
              ...
              >
                Animais:{' '}
                {patologia.prevalencia.animal.cachorro ? <PiCatLight /> : <></>}{' '}
                {patologia.prevalencia.animal.gato ? <PiDogLight /> : <></>}{' '}
              </Text>
              ...
              {sintomasNomes.map((nome) => (
                <Text onClick={() => mostrarChaveSintoma(patologia)} key={nome}>
                  {nome}
                </Text>
              ))}
            </Box>
